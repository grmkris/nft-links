...
export class NftLink extends ethereum.SmartContract {
  static bind(address: Address): NftLink {
    return new NftLink("NftLink", address);
  }
  ...
  mintNFT(recipient: Address, tokenURI: string): BigInt {
    let result = super.call("mintNFT", "mintNFT(address,string):(uint256)", [
  ...
  try_mintNFT(
    ...
    let result = super.tryCall("mintNFT", "mintNFT(address,string):(uint256)", [
      ethereum.Value.fromAddress(recipient),
      ethereum.Value.fromString(tokenURI)
    ]);
    if (result.reverted) {
