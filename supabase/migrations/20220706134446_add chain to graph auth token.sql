-- This script was generated by the Schema Diff utility in pgAdmin 4
-- For the circular dependencies, the order in which Schema Diff writes the objects is not very sophisticated
-- and may require manual changes to the script to ensure changes are applied in the correct order.
-- Please report an issue for any failure with the reproduction steps.

REVOKE ALL ON TABLE public.reward_groups FROM anon;
REVOKE ALL ON TABLE public.reward_groups FROM authenticated;
REVOKE ALL ON TABLE public.reward_groups FROM postgres;
REVOKE ALL ON TABLE public.reward_groups FROM service_role;
REVOKE ALL ON TABLE public.reward_groups FROM supabase_admin;
GRANT ALL ON TABLE public.reward_groups TO supabase_admin;

GRANT ALL ON TABLE public.reward_groups TO authenticated;

GRANT ALL ON TABLE public.reward_groups TO anon;

GRANT ALL ON TABLE public.reward_groups TO postgres;

GRANT ALL ON TABLE public.reward_groups TO service_role;

REVOKE ALL ON TABLE public.profiles FROM anon;
REVOKE ALL ON TABLE public.profiles FROM authenticated;
REVOKE ALL ON TABLE public.profiles FROM postgres;
REVOKE ALL ON TABLE public.profiles FROM service_role;
REVOKE ALL ON TABLE public.profiles FROM supabase_admin;
GRANT ALL ON TABLE public.profiles TO supabase_admin;

GRANT ALL ON TABLE public.profiles TO authenticated;

GRANT ALL ON TABLE public.profiles TO anon;

GRANT ALL ON TABLE public.profiles TO postgres;

GRANT ALL ON TABLE public.profiles TO service_role;

REVOKE ALL ON TABLE public.reward_nft FROM anon;
REVOKE ALL ON TABLE public.reward_nft FROM authenticated;
REVOKE ALL ON TABLE public.reward_nft FROM postgres;
REVOKE ALL ON TABLE public.reward_nft FROM service_role;
REVOKE ALL ON TABLE public.reward_nft FROM supabase_admin;
GRANT ALL ON TABLE public.reward_nft TO supabase_admin;

GRANT ALL ON TABLE public.reward_nft TO authenticated;

GRANT ALL ON TABLE public.reward_nft TO anon;

GRANT ALL ON TABLE public.reward_nft TO postgres;

GRANT ALL ON TABLE public.reward_nft TO service_role;

ALTER TABLE IF EXISTS public.graph_auth_tokens
    ADD COLUMN chain text COLLATE pg_catalog."default" DEFAULT 'polygon'::text;

REVOKE ALL ON TABLE public.user_groups FROM anon;
REVOKE ALL ON TABLE public.user_groups FROM authenticated;
REVOKE ALL ON TABLE public.user_groups FROM postgres;
REVOKE ALL ON TABLE public.user_groups FROM service_role;
REVOKE ALL ON TABLE public.user_groups FROM supabase_admin;
GRANT ALL ON TABLE public.user_groups TO supabase_admin;

GRANT ALL ON TABLE public.user_groups TO authenticated;

GRANT ALL ON TABLE public.user_groups TO anon;

GRANT ALL ON TABLE public.user_groups TO postgres;

GRANT ALL ON TABLE public.user_groups TO service_role;
