...
export default function CreateNftStep2SelectImage(props: { createNftForm, setCreateNftForm, nftMetadata, setNftMetadata }) {
  ...
  return (
    <div className="flex w-full flex-col items-center space-y-3 h-[500px] max-h-[500px] shadow-xl">
      ...
      <div className="grid place-items-center w-full">
        <label
          className="cursor-pointer	rounded-full px-2 py-3  text-center font-bold text-accent underline decoration-accent hover:text-accent-focus decoration-2 underline-offset-2  sm:w-1/3 md:w-1/2"
                ...
                <div className="carousel-center carousel rounded-box max-w-md space-x-4 bg-neutral p-4">
                  {ipfsFiles.data.map((file) => (
                    ...
                    >
                      <Image
                        onClick={() => {
                          props.setNftMetadata({...props.nftMetadata, image: file.id})
