...
export const getServerSideProps : GetServerSideProps<definitions["reward_program"] &
  {reward_groups: Array<{groups: definitions["groups"] & {user_groups: Array<definitions["user_groups"]>}}>}
  ...
  const reward_program = await supabaseServerClient.from<definitions["reward_program"] &
    {reward_groups: Array<{groups: definitions["groups"] & {user_groups: Array<definitions["user_groups"]>}}>}
    & {reward_nft: Array<definitions["reward_nft"] & {nft: definitions["nfts"]}>}
    >('reward_program')
    .select(`
...
function ViewReward(data: InferGetServerSidePropsType<typeof getServerSideProps>) {
  ...
  const uploadToServer = async () => {
    ...
    if (result.data.claimed)
      toast.success("Success! Task completed successfully.")
    else {
